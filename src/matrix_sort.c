...
void quick_sort(int **matrix, int rows, int columns) {
    qs(matrix, rows, columns, 0, rows - 1);
}
...
void qs(int **matrix, int rows, int columns, int left, int right) {
    ...
    do {
        while ((i < right) && (sum_row(matrix[i], columns) < x))
            i++;

        while ((j > left) && (x < sum_row(matrix[j], columns)))
            j--;
    ...
    if (left < j)
        qs(matrix, rows, columns, left, j);

    if (i < right)
        qs(matrix, rows, columns, i, right);
}
